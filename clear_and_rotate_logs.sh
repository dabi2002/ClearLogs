...
process_logs() {
    ...
    for limspLog in "${limspLogsNames[@]}"; do
        ...
        if [[ ${#logFilesArray[@]} -gt 0 ]]; then
            tar czf "${logPath}/${fechaCorta}/limsp-logs-${logPath}.tgz" "${logFilesArray[@]}"
            if [[ $? -eq 0 ]]; then
                rm -f "${logFilesArray[@]}"
            fi
        fi
...
find . -maxdepth 1 -type d -name "20??????" -exec basename {} \; | grep -o '[0-9]\{8\}' | while read dayPath; do
    # Verificar si dayPath está vacío
    if [ -z "$dayPath" ]; then
        continue  # Saltar esta iteración si dayPath está vacío
    fi
    
    # Convertir dayPath a un entero
    declare -i dayPath
...
ssh_session(){
    ...
    ssh -o ForwardX11=no "$username"@"$nodo" "
        ...
        process_logs "$logPath" "$dirName" "$historicPath" "$archiveDays" "$fechaCorta" # Llamar a la función process_logs dentro de la sesión SSH
    "
}
...
for nodo in "${!nodos[@]}"; do
    ...
    for dirName in "${!logPaths[@]}"; do
        ...
        ssh_session "$nodeName" "$logPath" "$dirName" "$historicPath" "$archiveDays" "$fechaCorta"
    done
done
